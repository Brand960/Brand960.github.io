...
academia: true
---
# stm32h750vbt6 软件启动流程1（as）

>本文档针对使用 arm-gcc-toolchain 工具链， stm32h750vbt6 CubeMX 生成的 `startup_stm32h750xx.s`
<!-- more -->
## 零、整体流程
...
│  ① ldr sp, =_estack        ← 设置栈指针（软件显式初始化）     │
│  ② bl  ExitRun0Mode        ← 退出 Run* 模式进入 Run 模式     │
│  ③ bl  SystemInit          ← 初始化系统（FPU/VTOR/外存接口）  │
...
声明目标处理器、浮点调用约定与指令集，选择 Thumb 指令编码
...
```asm
  ...
  .fpu softvfp         ; 声明本汇编文件不使用硬件 FPU 指令
...
| `.cpu cortex-m7`  | 指定目标 CPU                         | 同上（`.cpu`）                                    |
| `.fpu softvfp`    | 软浮点调用约定                       | 同上（`.fpu`）                                    |
| `.thumb`          | 以 Thumb 指令集编码，等价 `.code 16` | 同上（`.thumb`）                                  |

### 依据

- `.thumb` 使函数地址 LSB=1、复位后进入 Thumb 状态，对应复位序列将 EPSR.T 位置为复位向量 bit[0]
- 复位后硬件据此 `BranchTo` 到 `Reset_Handler`（**ARMv7-M ARM** Reset 伪代码）。
- GCC 的 -mfpu=fpv5-d16 -mfloat-abi=hard 决定 ABI/调用约定，结合 SystemInit 中对 CPACR.CP10/CP11 = 0b11 的置位使能协处理器让硬件浮点生效

## 二、全局符号声明

导出向量表起始符号与默认中断处理函数两个全局符号，以及由链接脚本解析的下划线符号（该处为ST CubeMX 启动模板中的冗余声明，真正符号值来自.ld脚本）
...
Reset_Handler:
  ...
  bl  ExitRun0Mode                    ; 调用电源模式配置(STM32H750VBT6 默认 LDO 模式使用弱定义空函数，兼容性保留)
...
- 设栈（`ldr sp, =_estack`）对应硬件复位序列已从向量表首项自动载入 MSP（**ARMv7-M ARM** Reset 伪代码），与硬件无关的显式初始化，便于调试/移植/审计和作为ST模板的保险，与硬件加载互补
...
```asm
  .section  .text.Default_Handler,"ax",%progbits  ; 可执行输入段（a=可分配 SHF_ALLOC，x=可执行 SHF_EXECINSTR，%progbits=SHT_PROGBITS）
...
g_pfnVectors:                            ; 向量表起始
  ...
  …                                      ; 偏移 0x40 起: 外设 IRQ（共150槽位（RM0433 §中断与事件/向量表表））
```

### 依据

- 偏移 0 为 MSP、偏移 +4 为复位向量的取值顺序，依据 ARMv7-M 异常向量布局（**ARMv7-M ARM** Exception Model）。
- H7 的启动地址由选项字节 BOOT_ADD0/BOOT_ADD1（16 位，目标地址高 16 位）+ 复位时 FLASH/SYSTEM boot 选择决定（**RM0433** FLASH 章节 Boot configuration / FLASH_BOOT_CURR；**DS12556** §3.4 Boot modes）。
...
| 伪指令            | 含义                                                                      | 来源                                               |
| ----------------- | ------------------------------------------------------------------------- | -------------------------------------------------- |
| `.thumb_set X, Y` | 在 ST 启动模板里把弱符号设为另一个符号的别名（与 .weak 联用形成“弱别名”） | **as** §7.89 [.set]；§9.4.4 ARM Machine Directives |

## 附录

- **as** = `Using as`（GNU Binutils v2.46）
- **ld** = `GNU linker ld`（GNU Binutils v2.46）
- **ELF** = `ELF ABI 规范手册`（Xinuos ELF v4.2）
